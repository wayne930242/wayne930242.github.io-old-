...
**A2. $\mathcal{L}_\mathbf{K}$ 蒐集所有的句式（well-formed formula, *wff*），構成規則（formation rules）如下：**

* (F1) 每個語句字母都是一個句式。（原子句式）
* (F2) 如果 $\phi$ 是一個句式，那 $\lnot \phi$ 也是。
* (F3) 如果 $\phi$ 和 $\psi$ 都是句式，那 $(\phi \lor \psi)$、$(\phi \land \psi)$、$(\phi \to \psi)$、$(\phi \leftrightarrow \psi)$ 也是。
* (F4) 除此之外，沒有其他句式。[^3]
...
若句式序列 $\\{  \phi_1, \phi_2, \cdots , \phi_n \\}$ 被稱作句式 $\phi$ 的構成序列，那：$\phi_n = \phi$ ，[^4]並且對於每個 $k \le n$，恰有以下之一被滿足：
...
**B2. 定義 1.2：子句式（subformula）**

若句式 $\psi$ 被稱作句式 $\phi$ 的一個子句式，則有以下之一被滿足：

1. $\psi=\phi$ 
2. $\phi=\lnot\psi_i$ 並且 $\psi$ 是 $\psi_i$ 的一個子句式。
3. $\phi$ 符合以下形式之一： $(\psi_i \lor \psi_j)$、$(\psi_i \land \psi_j)$、$(\psi_i \to \psi_j)$、$(\psi_i \leftrightarrow \psi_j)$，其中 $\psi$ 是 $\psi_i$ 或 $\psi_j$ 的一個子句式。

**B3. 定義 1.3：直接子句式（immediate subformula）** 

若句式 $\psi$ 被稱作句式 $\phi$ 的一個直接子句式，則有以下之一被滿足：

1. $\phi=\lnot\psi$ 。
2. $\phi$ 符合以下形式之一： $(\psi_i \lor \psi_j)$、$(\psi_i \land \psi_j)$、$(\psi_i \to \psi_j)$、$(\psi_i \leftrightarrow \psi_j)$，其中 $\psi = \psi_i$ 或 $\psi = \psi_j$ 。

**B4. 定義 1.4：句式的主要連接符（main connective）**

如果一個連接符連接了 $\phi$ 的直接子句式，我們就稱此連接符為 $\phi$ 的主要連接符。

**B5. 定義 1.5：連接符的出現的範圍（the *scope* of an occurrence of a connective）**

給定一個句式 $\phi$ ，一個連接符的出現的範圍為 $\phi$ 的一個子句式 $\psi$ ，滿足：

1. 該連接符的出現在 $\psi$ 中；
2. 對於所有 $\psi$ 的直接子句式 $\theta$ ，該連接符的出現不在 $\theta$ 中。

**B6. 定理 1.1：唯一可讀性定理（分解的唯一性定理）**

如果 $\phi$ 是 $\mathcal{L}$ 的一個句式，以下之一恰好成立：
...
**[證明方法]** 先定義句式長度的度量方法（一個從句式對應到自然數的函數），並以數學歸納法證明，對於任何長度的句式這性質都成立。
...
1. **二值原則（the principle of bivalence）**：句式的真值只有兩種，真或假。
2. **真值函數性原則（the principle of truth-functionality）**：連接符必須是真值函數（又稱為布林函數， Boolean function）。
3. **擴充性原則（the principle of extensionality）**：任何句式的真值只能由包含其中的原子句式之真值與連接符決定。
...
一個句式 $\phi$ 的真值如果在 $\mathscr{M}$ 中被決定，我們就說， $\phi$ 在 $\mathscr{M}$ 中被定義。
...
對於一個句式集合 $\Gamma$ ，如果不存在任何結構使得 $\Gamma$ 的所有句式被定義且為真，我們就說， $\Gamma$ 是語義地不一致的（semantically inconsistent），符號寫作 $\Gamma\models$ 。否則就說， $\Gamma$ 是語義地一致的。

**D2. 定義 1.9：語義蘊含關係**

(1) 如果不存在任何結構，使得句式 $\phi$ 為假且使得句式集合 $\Gamma$ 中的所有句式為真，我們就說， $\Gamma$ 語義地蘊含 $\phi$ ，符號寫作：$\Gamma \models \phi$。
...
說 $\mathscr{M}$ 是序列 $\Gamma \models \phi$ 的一個反例，若且唯若， $\Gamma$ 中的每個句式在 $\mathscr{M}$ 中為真，並且 $\phi$ 在 $\mathscr{M}$ 為假。
...
若 $\Gamma \models \phi$ ，則，對於所有原子句式 $P$ 和句式 $\psi$，$\Gamma (\psi/P) \models \phi (\psi/P)$ 。 

符號說明：$\Gamma(\psi/P)$ 意味著將 $\Gamma$ 中每個句式中出現的所有 $P$ 置換為 $\psi$ ， $\phi(\psi/P)$ 意味著將 $\phi$ 中出現的所有 $P$ 置換為 $\psi$ 。
...
符號說明：$\psi^* $ 被稱作 $\psi $ 的對偶， $* $ 是一個函數，將句式中所有的 $\lor$ 換成 $\land$ ，並將所有的 $\land$ 換成 $\lor$ ，如此有 $\phi^* = *(\phi)$ 。

**E.6 定理 1.5 的引理**

對於任意的 $\phi$ ，假定 $\phi$ 只由原子句式 $P_1, P_2, \cdots P_n$ 所構成，則 $\phi^*$ 與 $\lnot\phi(\lnot P_1/P_1 , \cdots , \lnot P_n/P_n)$ 是邏輯等同的。

# NOTES

[^1]: 又稱真值函數子/運算子（truth-functors/operators）。

[^2]: 在波蘭記號系統中，(F2)與(F3)會如此改寫：(P2)如果 $\phi$ 是一個句式，那 $N\phi$ 也是。(P3)如果 $\phi$ 和 $\psi$ 都是句式，那 $(A\phi\psi)$（$\lor$）、$(K\phi\psi)$（$\land$）、$(C\phi\psi)$（$\to$）、$(E\phi\psi)$ （$\leftrightarrow$）也是。

[^3]: 我們通常把 $\top$ 和 $\bot$ 作為額外的主要符號。但在這裡，它們是作為語句字母被加進來， $\top$ 定義為「在真值表上總是為真的語句」，而 $\bot$ 定義為「在真值表上總是為假的語句」。

[^4]: 我以 $=$ 表示句式的同一關係，一種等同關係（equivalance relation）。

[^5]: 在布林代數下，我們將對句式 $\phi$ 的分派以賦值函數加以定義。賦值函數是將句式 $\phi$ 對應到 $\\{ 0, 1 \\}$ 的函數 $v$ ，遞迴地以 $\phi$ 之直接子句式的函數值來定義句式的函數值，直到作為直接子句式的原子句式的真值被 $v$ 初始且任意地給定。我們將這稱作一個布林賦值（Boolean valuation）。如果存在賦值函數使得 $\phi$ 對應到 $1$ ，則我們說 $\phi$ 是可滿足的。

[^6]: $\Gamma, \lnot\phi \models \psi$ 為 $\Gamma\cup \\{ \lnot\phi \\} \models \psi$ 的標準簡寫。同樣地，如果 $\Delta$ 是另一個句式集合，我們用 $\Gamma, \lnot\Delta \models \psi$ 簡寫 $\Gamma\cup \Delta \models \psi$ 。
